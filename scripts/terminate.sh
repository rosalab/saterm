...
WORKLOAD_PIDS="$CPU_WORKLOAD_PID $MEM_WORKLOAD_PID $IO_WORKLOAD_PID"

# Step 2: Compile the binaries
echo "Compiling BPF binaries..." >> "$LOGFILE"
cd /linux-dev-env/bpf-progs || exit
make clean
make -j"$(nproc)"

# Step 3: Run the binaries in sequence
echo "Starting /linux-dev-env/bpf-progs/loops.user in background..." >> "$LOGFILE"
./load_syscall.user &  # Run first binary in background
sleep 1         # Wait for 1 second
echo "Starting /linux-dev-env/bpf-progs/trigger_loops.user in background..." >> "$LOGFILE"
./trigger.user &  # Run second binary in background
